...
const statusConfig: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
  ...
  PENDING_MAIN_APPROVAL: { icon: <Clock size={14} />, label: 'Awaiting Main Admin', color: 'text-blue-400 bg-blue-400/10' },
...
// Read shopId synchronously from persisted zustand store in localStorage
const getShopIdFromStorage = (): number | null => {
  try {
    const raw = localStorage.getItem('admin-auth-storage');
    if (!raw) return null;
    const shopId = JSON.parse(raw)?.state?.user?.shopId;
...
const ShopApplications = () => {
  ...
  const { data: pageData, isLoading } = useQuery({
    queryKey: ['shopAdminApplications', shopId],
    queryFn: () => getShopAdminApplications(shopId!, 0, 50),
    enabled: !!shopId,
  });
  ...
  const confirmReject = () => {
    ...
    if (selectedId) rejectMutation.mutate(selectedId);
  };
  ...
  const renderPhotos = (app: ApplicationResponseDTO) => {
    ...
    if (app.profilePictureUrl) docs.push({ url: app.profilePictureUrl, label: 'Profile' });
    if (app.licenseUrl) docs.push({ url: app.licenseUrl, label: 'License' });
        ...
        <div className="flex gap-6 h-[calc(100vh-220px)]">
          <div className="flex-1 space-y-2 overflow-y-auto pr-2">
            {isLoading ? (
              ...
              <div className="text-center text-muted-foreground py-10">No applications found</div>
            ) : (
              filtered.map(app => {
