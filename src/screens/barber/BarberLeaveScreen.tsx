// src/screens/barber/BarberLeaveScreen.tsx
...
const seedLeaveRequests: LeaveRequest[] = [
  { id: '1', barberId: 'b1', barberName: 'Marcus Johnson', shopId: 's1', startDate: '2026-02-20', endDate: '2026-02-20', reason: 'Doctor appointment', status: 'APPROVED', appliedAt: '2026-02-10', reviewedAt: '2026-02-11' },
  { id: '2', barberId: 'b1', barberName: 'Marcus Johnson', shopId: 's1', startDate: '2026-03-05', endDate: '2026-03-06', reason: 'Family event', status: 'PENDING', appliedAt: '2026-02-15' },
];
...
const DatePickerInput = ({ 
  ...
  // Removed the explicit type annotation : DateCallbackHandler
...
const BarberLeaveScreen = () => {
  const [leaves, setLeaves] = useState<LeaveRequest[]>(seedLeaveRequests.filter(l => l.barberId === 'b1'));
  ...
  const validate = () => {
    ...
    if (startDate && endDate) {
      const overlapping = leaves.find(
        l => l.status === 'APPROVED' &&
          !(endDate < l.startDate || startDate > l.endDate)
      );
      if (overlapping) {
        errs.startDate = `Overlaps with approved leave`;
      }
    }
  ...
  const handleSubmit = () => {
    ...
    setReason('');
    setErrors({});
    Alert.alert('Leave Applied', 'Your leave request has been submitted for review.');
  };

  const handleCancel = (id: string) => {
    setLeaves(prev => prev.filter(l => l.id !== id));
    Alert.alert('Leave Cancelled', 'Your pending leave request has been withdrawn.');
  };
  ...
  const getStatusStyle = (status: LeaveStatus) => {
    switch (status) {
      case 'PENDING':
        return { bg: 'rgba(250, 204, 21, 0.1)', color: '#FACC15', border: 'rgba(250, 204, 21, 0.2)' };
      case 'APPROVED':
      ...
      default:
        return { bg: 'transparent', color: theme.colors.text, border: 'transparent' };
    }
  };
  ...
  const getStatusIcon = (status: LeaveStatus) => {
    ...
    switch (status) {
      case 'PENDING': return <Clock size={12} color={color} />;
      case 'APPROVED': return <CheckCircle size={12} color={color} />;
      case 'REJECTED': return <XCircle size={12} color={color} />;
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity 
          style={[styles.headerBtn, showForm && styles.headerBtnCancel]} 
          onPress={() => setShowForm(!showForm)}
        >
          {showForm ? (
             <X size={14} color={showForm ? theme.colors.text : theme.colors.primaryText} />
          ) : (
             <Plus size={14} color={theme.colors.primaryText} />
          )}
          <Text style={[styles.headerBtnText, showForm && { color: theme.colors.text }]}>
      ...
      <View style={styles.statsGrid}>
        {[
          { label: 'Total Requests', value: stats.total, color: theme.colors.text },
          ...
          return (
            <View key={leave.id} style={styles.leaveCard}>
                ...
                <View style={styles.leaveInfoRow}>
                  ...
                  <View style={styles.leaveTextContainer}>
                    <Text style={styles.leaveDateText}>
                      {isSingleDay ? leave.startDate : `${leave.startDate} → ${leave.endDate}`}
                    </Text>
              ...
              <View style={styles.leaveFooter}>
                <Text style={styles.footerText}>Applied: {leave.appliedAt}</Text>
                <View style={styles.footerActions}>
                  {leave.reviewedAt && <Text style={styles.footerText}>Reviewed: {leave.reviewedAt}</Text>}
                  {leave.status === 'PENDING' && (
                    <TouchableOpacity onPress={() => handleCancel(leave.id)}>
              ...
              {leave.adminNotes && (
                <View style={styles.adminNoteContainer}>
                  <Text style={styles.adminNoteText}>Admin: {leave.adminNotes}</Text>
                </View>
              )}
            </View>
          );
